...
def _criar_cliente(client, email, valor):
    return client.post(
        "/clientes",
        json={
...
def test_webhook_aplica_regra_de_prioridade(client, valor, prioridade_esperada):
    email = f"cliente{valor}@example.com"
    _criar_cliente(client, email, valor)
...
def test_webhook_event_id_duplicado_e_bloqueado(client):
    email = "joao.silva@example.com"
    _criar_cliente(client, email, 250000)
    ...
    primeira = client.post("/webhooks/pipefy/card-updated", json=payload)
    assert primeira.status_code == 200
    assert primeira.json()["cliente"]["status"] == "Processado"

    # Reentrega do mesmo evento: não reprocessa.
    segunda = client.post("/webhooks/pipefy/card-updated", json=payload)
    assert segunda.status_code == 200
    assert "já processado" in segunda.json()["detail"]
    assert segunda.json()["cliente"] is None


def test_webhook_cliente_inexistente_retorna_404(client):
    resp = client.post(
        "/webhooks/pipefy/card-updated",
        json={
            "event_id": "evt_999",
